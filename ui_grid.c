...
void ui_grid_draw(struct ui_grid *grid, double scale, double x0, double y0, double x1, double y1) {
  ...
  GLuint vcap = 2 * (ceil((x1 - x0) / gap_x) + ceil((y1 - y0) / gap_y) + ceil((x1 - x0) / (gap_x * 10)) + ceil((y1 - y0) / (gap_y * 10)));
  ...
  int i;
  unsigned char r = 255 * grid->color[0] * coeff;
  unsigned char g = 255 * grid->color[1] * coeff;
  unsigned char b = 255 * grid->color[2] * coeff;
  unsigned char a = 255 * grid->color[3];
