...
static LRESULT CALLBACK wndproc(HWND window, UINT msg, WPARAM wparam, LPARAM lparam)
{
        ...
        static int wheel_accum_y = 0;
        static int wheel_accum_X = 0;
        ...
        case WM_SIZE:
                if (w)
                {
                        w->cb_vtable_p->size(w, LOWORD(lparam), HIWORD(lparam), w->data);
                }
                return 0;
        case WM_MOUSEMOVE:
                if (w)
        ...
        case WM_MOUSELEAVE:
                if (w)
                        w->cb_vtable_p->mouse_leave(w, w->data);

                return 0;
        case WM_MOUSEWHEEL: {
                wheel_accum_y += GET_WHEEL_DELTA_WPARAM(wparam);

                int steps = wheel_accum_y / WHEEL_DELTA;
        ...
        case WM_MOUSEHWHEEL: {
                wheel_accum_X += GET_WHEEL_DELTA_WPARAM(wparam);

                int steps = wheel_accum_X / WHEEL_DELTA;
...
{
        PostMessage(nw->w, WM_APP_RENDER, 0, 0);
        return 1;
}
...
{
        SetWindowPos(nw->w, NULL, 0, 0, width, height, SWP_NOZORDER);
        return 1;
}
