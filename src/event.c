...
#define EVT(obj) ((struct gllc_event *)(obj))
...
static struct gllc_object_vtable _vtable = {.type = GLLC_EVENT, .destroy = _destroy};
...
int gllc_events_cleanup()
{
        struct _proc *proc = _handlers;
        while (proc)
        {
                struct _proc *next = proc->next;
                free(proc);
                proc = next;
        }
        _handlers = NULL;
