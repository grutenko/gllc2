...
import os.path
litecad = ct.WinDLL(os.path.join(os.path.dirname(__file__), "./litecad64.dll"))
...
LC_EM_TOP =       0  #  top entity
LC_EM_ENT =       1  #  entity to explode
LC_EM_PLINE =     2  #  begin of entity polyline
LC_EM_VERTEX =    3  #  vertex of polyline
LC_EM_ENDPLINE =  4  #  end of entity polyline
LC_EM_ENDENT =    5  #  end of entity to explode
...
LC_DRWEXP_BLOCKS = 20   #  explode all blocks
...
LC_CMD_SHDOTS =         137   #  on/off shape dots (used for testing)
LC_CMD_SMALLVRECT =     138   #  display reduced view (used for testing)
...
LC_CMD_TIN_BNDPLINE =   328   #  TIN: 
LC_CMD_TIN_Z =          329   #  TIN: display TIN Z-coord near cursor
LC_CMD_TIN_ZOOM =       330   #  TIN: zoom extents
LC_CMD_TIN_ISOTODRW =   331   #  TIN: copy isolines into drawing
LC_CMD_TIN_TILES =      332   #  TIN: draw TIN tiles
...
LC_PROP_WND_DROPFILES =       329    #  RW bool; Registers whether a window accepts dropped files
LC_PROP_WND_ZOOMWHEEL =       330    #  RW bool; enable zoom by wheel
...
LC_PROP_WND_JUMPLINES =       339    #  RW bool; display jump lines
LC_PROP_WND_MAGNIFIER =       340    #  RW bool; display magnifier at window rig-bot corner
LC_PROP_WND_NAVIGATOR =       341    #  RW bool; display "Aerial View" window
LC_PROP_WND_COLORBG =         342    #  RW int; background color
LC_PROP_WND_COLORCURSOR =     343    #  RW int; cursor color
LC_PROP_WND_COLORFORE =       344    #  RW int; foreground color
LC_PROP_WND_COLORINFBG =      345    #  RW int; info box background color
LC_PROP_WND_COLORINFBORD =    346    #  RW int; info box border color
LC_PROP_WND_COLORINFTEXT =    347    #  RW int; info box text color
LC_PROP_WND_CURSORSYS =       348    #     same as LC_PROP_WND_CURSORARROW
LC_PROP_WND_CURSORARROW =     348    #  RW bool|int|handle; Enable arrow cursor | set cursor (LC_CURSOR_ARROW or other)
LC_PROP_WND_CURSORCROSS =     349    #  RW bool; Enable crosshair cursor
LC_PROP_WND_CURSORSIZE =      350    #  RW int; Size of crosshair cursor, % of screen, if negative - size in pixels
LC_PROP_WND_CURSORPBOX =      351    #  RW bool; Enable cursor pickbox rect
LC_PROP_WND_COORDS =          352    #  RW bool; display cursor coords at left-bottom corner
LC_PROP_WND_ENT =             353    #  R  handle; picked entity (see lcWndPickEnt function)
LC_PROP_WND_PROPWND =         354    #  R  handle; Handle to properties window
LC_PROP_WND_BLOCKBPT =        355    #  RW bool; display block basepoint
LC_PROP_WND_LWMODE =          355    #  RW int;  line width mode (LC_LW_THIN, LC_LW_REAL, LC_LW_PIXEL)
LC_PROP_WND_LWSCALE =         356    #  RW float; line width screen scale, mm / pixel
...
LC_PROP_WND_ALPHABLEND =      360    #  RW bool; enable alpha blend (transparent filling)
LC_PROP_WND_STDBLKFRAME =     361    #  RW bool; draw red frame around the window if standard block is active
LC_PROP_WND_BLKBASEPT =       362    #  RW bool; display block basepoint
LC_PROP_WND_SIZE =            363    #  R  string; window size, pixels (format "%d x %d", PSWidth(), PSHeight() )
LC_PROP_WND_DTIME =           364    #  R  int; redraw time, milliseconds
LC_PROP_WND_DRAWPAPER =       365    #  RW bool; draw paper sheet for "Paper space" blocks
LC_PROP_WND_FROZEN =          366    #  RW bool; "Frozen" mode
LC_PROP_WND_FROZENVIEW =      367    #  RW bool; if true - can't chnage view rect
LC_PROP_WND_COMMAND =         368    #  R  int|handle|bool; Id of active command | handle to active command | TRUE if has active command
LC_PROP_WND_CMD =             368    #  same as LC_PROP_WND_COMMAND
LC_PROP_WND_CMDENT1 =         369    #  RW bool; TRUE - only one entity added by command, FALSE - several entities
LC_PROP_WND_OSNAP =           370    #  RW int|bool; object snap mode  LC_OSNAP_NODE and others | On/Off
LC_PROP_WND_OSNAPMENU =       371    #  RW bool; display "object snap" menu on <shift>+<RBDown> (CBaseWnd::BaseOnRButtonDown)
LC_PROP_WND_ORTHO =           372    #  RW bool; ortho mode
LC_PROP_WND_PTRACK =          373    #  RW bool; polar tracking on/off
LC_PROP_WND_PTRACK_ANGLE =    374    #  RW float; step angle for polar tracking
LC_PROP_WND_PTRACK_ANGREL =   375    #  RW bool; relative / absolute angle
LC_PROP_WND_PTRACK_DIST =     376    #  RW bool|float; use distance step along polar vector | value of dist. step
LC_PROP_WND_BASEPT =          377    #  R  bool; Has Base point   // undoc
LC_PROP_WND_BASEX =           378    #  R  float; Base point position X   // undoc
LC_PROP_WND_BASEY =           379    #  R  float; Base point position Y   // undoc
LC_PROP_WND_GRIDSNAP =        380    #  RW bool; 
LC_PROP_WND_GRIDSHOW =        381    #  RW bool; Display coordinate grid
LC_PROP_WND_GRIDDX =          382    #  RW float; Grid & snap step
LC_PROP_WND_GRIDDY =          383    #  RW float;
LC_PROP_WND_GRIDX0 =          384    #  RW float; Origin point
LC_PROP_WND_GRIDY0 =          385    #  RW float;
LC_PROP_WND_GRIDBOLDX =       386    #  RW int; Bold step X
LC_PROP_WND_GRIDBOLDY =       387    #  RW int; Bold step Y
LC_PROP_WND_GRIDCOLOR =       388    #  RW int; Line color
LC_PROP_WND_GRIDDOTTED =      389    #  RW bool; Line is dotted
LC_PROP_WND_GRIDCOLOR2 =      390    #  RW int; Bold Line color
LC_PROP_WND_GRIDDOTTED2 =     391    #  RW bool; Bold Line is dotted
LC_PROP_WND_RSNAP =           402    #  RW bool; Enable snap to rectangle border
LC_PROP_WND_RSNAPSHOW =       403    #  RW bool; Display snap rectanle
LC_PROP_WND_RSNAPLEF =        404    #  RW float; X left
LC_PROP_WND_RSNAPBOT =        405    #  RW float; Y bottom
LC_PROP_WND_RSNAPRIG =        406    #  RW float; X right
LC_PROP_WND_RSNAPTOP =        407    #  RW float; Y top
LC_PROP_WND_RSNAPW =          408    #  RW float; Width (from X left)
LC_PROP_WND_RSNAPH =          409    #  RW float; Height (from Y bottom)
LC_PROP_WND_RSNAPCOLORM =     410    #  RW int; Rectangle filling color on Model space
LC_PROP_WND_RSNAPCOLORP =     411    #  RW int; Rectangle filling color on Paper space
LC_PROP_WND_PANSTEP =         420    #  RW int; Minimal step, pixels
LC_PROP_WND_PANLW =           421    #  RW bool; optimize line width (draw as 1pix size)
LC_PROP_WND_PANIMAGE =        422    #  RW bool; optimize raster images (draw as hatch)
LC_PROP_WND_PANFILL =         423    #  RW bool; optimize polygon filling (don't fill)
LC_PROP_WND_PANPIXSZ =        424    #  RW bool; reduce resolution (increase pixel size)
LC_PROP_WND_MEASCOLORPNT =    430    #  RW int; COLORREF for points of DIST, AREA tools
LC_PROP_WND_MEASCOLORLINE =   431    #  RW int; COLORREF for lines of DIST, AREA tools
LC_PROP_WND_MEASLINESIZE =    432    #  RW int; line size for DIST, AREA tools
LC_PROP_WND_MEASFONTSIZE =    433    #  RW int; font size
LC_PROP_WND_MEASFILLAREA =    434    #  RW int; fill area polygons with hatch
LC_PROP_WND_KBMOVE_ENABLE =   437    #  RW bool; enable movement by keyboard
LC_PROP_WND_KBMOVE_DIST =     438    #  RW float; distance step
LC_PROP_WND_KBMOVE_ANGLE =    439    #  RW float; angle step
LC_PROP_WND_KBMOVE_SCALE =    440    #  RW float; scale step
LC_PROP_WND_XLINEANG =        444    #  RW float; direction angle for command LC_CMD_XLINE
LC_PROP_WND_BGIMAGE =         445    #  RW string|bool; Background image filename | Display
LC_PROP_WND_LTVIEWMIN =       446    #  RW int; Linetype visibility, min size of pattern, pixels
LC_PROP_WND_LTVIEWMAX =       447    #  RW int; Linetype visibility, max size of pattern, pixels
LC_PROP_WND_FRAME_LEFT =      451    #  R  int; Left position on parent window (pixels)
LC_PROP_WND_FRAME_TOP =       452    #  R  int; Top position on parent window (pixels)
LC_PROP_WND_FRAME_WIDTH =     453    #  R  int; Frame width (pixels)
LC_PROP_WND_FRAME_HEIGHT =    454    #  R  int; Frame height (pixels)
...
LC_PROP_DRW_PBTEXT =          3027    #  RW string; Text of progress box
...
LC_PROP_POINT_Z =          5061  #  RW float; same as LC_PROP_ENT_Z
...
LC_PROP_BLKREF_GRIPMODE =  5545  #  RW int;
...
# typedef void (CALLBACK* F_EXP) (int Mode, HANDLE hEntity, int nPaths, int iPath, int nPnts, int iPnt, double X, double Y);
...
lcWndExitCommand.restype = wt.BOOL
...
lcDrwUpdateTexts.restype = wt.BOOL
lcDrwUpdateTexts.argtypes = [wt.HANDLE, wt.HANDLE]
...
lcExpBlock.restype = wt.BOOL
lcExpBlock.argtypes = [wt.HANDLE, ct.c_void_p, ct.c_int, wt.HANDLE]
